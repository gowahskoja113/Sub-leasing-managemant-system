...
Khi **cả hai** = `true` → BE set `paymentStatus = PAID`. Manager gọi `/send-otp` khi sẵn sàng gửi mã.
...
- Manager bấm **Gửi OTP** → `POST /send-otp` (Twilio Verify tới SĐT khách)
...
| Sau PAID | Nút **Gửi OTP** → `/send-otp` (QR và tiền mặt) |
...
| Gửi OTP | Manager **thủ công** `/send-otp` | Manager **thủ công** `/send-otp` |
