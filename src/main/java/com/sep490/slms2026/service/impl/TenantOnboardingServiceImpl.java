...
public class TenantOnboardingServiceImpl implements TenantOnboardingService {
    ...
    public TenantContractResponse onboardTenant(Long propertyId, Long roomId, OnboardTenantRequest request) {
        ...
        TenantContract contract = TenantContract.builder()
                ...
                .status(request.isRequireDepositPayment() || request.isRequireHostPriceApproval() ? ContractStatus.PENDING : ContractStatus.ACTIVE)
        ...
        // Chỉ set phòng RENTED khi HĐ kích hoạt ngay (không yêu cầu thanh toán cọc trước và không chờ duyệt giá).
        // Với luồng có thanh toán hoặc chờ duyệt: phòng sẽ được set RENTED ở bước confirm.
        if (room != null && !request.isRequireDepositPayment() && !request.isRequireHostPriceApproval()) {
            room.setStatus(RoomStatus.RENTED);
            roomRepository.save(room);
        }

        if (!request.isRequireDepositPayment() && !request.isRequireHostPriceApproval()) {
            autoGenerateDocument(saved.getId());
        }
    ...
    public TenantContractResponse confirmContract(Long contractId, String otp) {
        ...
        return toResponse(saved, contract.getTenant().getUser().getUsername(), false, false);
    }
    ...
    public TenantContractResponse syncPaymentStatus(Long contractId) {
        ...
        if (contract.getPaymentStatus() != PaymentStatus.PAID && contract.getPayosOrderCode() != null) {
            ...
            if ("PAID".equalsIgnoreCase(status)) {
                ...
                autoGenerateDocument(contract.getId());
            }
        }
        return toResponse(contract);
    }
    ...
    @Override
    @Transactional
    public List<TenantContractResponse> getManagedContracts(com.sep490.slms2026.enums.PriceApprovalStatus status) {
        ...
        if (status != null) {
            contracts = tenantContractRepository.findManagedContractsByApprovalStatus(managerUserId, status);
        } else {
    ...
    private void autoGenerateDocument(Long contractId) {
        try {
            tenantContractDocumentService.generateAndStore(contractId);
        } catch (Exception ex) {
            log.warn("Tự động xuất file hợp đồng thất bại contractId={}: {}", contractId, ex.getMessage());
        }
    }
