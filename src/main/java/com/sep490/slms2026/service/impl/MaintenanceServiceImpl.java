...
public class MaintenanceServiceImpl implements MaintenanceService {
    ...
    public Page<MaintenanceRequestResponse> getRequests(
        ...
        Specification<MaintenanceRequest> spec = (root, query, cb) -> {
            ...
            if (companyAbsorbedFault != null) {
                predicates.add(cb.equal(root.get("companyAbsorbedFault"), companyAbsorbedFault));
            }
            if (from != null) {
    ...
    public MaintenanceRequestResponse createRequest(MaintenanceCreateRequest request) {
        ...
        if (room != null) {
            locationLabel = "phòng " + room.getRoomNumber();
        } else if (equipment != null && equipment.getRoom() != null) {
            locationLabel = "phòng " + equipment.getRoom().getRoomNumber();
        } else {
            locationLabel = "nguyên căn " + property.getPropertyName();
        }
    ...
    public MaintenanceRequestResponse approve(Long id, MaintenanceApproveRequest request) {
        ...
        if (repairAt != null || Boolean.TRUE.equals(request.getNeedsOffSiteInspection())) {
            ...
            repository.save(req);
            String note = repairAt != null ? "Manager duyệt [" + category + "], đặt lịch sửa " + repairAt : "Manager đánh dấu mang đi kiểm tra thêm";
            addTimeline(req, old, MaintenanceStatus.REPAIR_SCHEDULED, note);
            notifyTenant(req,
    ...
    public MaintenanceRequestResponse rejectFault(Long id, MaintenanceRejectFaultRequest request) {
        ...
        if (request.getResolutionPath() == FaultResolutionPath.TENANT_SELF_REPAIR) {
            ...
            if (request.getRepairAppointmentAt() != null || Boolean.TRUE.equals(request.getNeedsOffSiteInspection())) {
                ...
                req.setStatus(MaintenanceStatus.REPAIR_SCHEDULED);
                repository.save(req);
                String note = request.getRepairAppointmentAt() != null ? "Manager xác định lỗi tenant — hẹn sửa lúc " + request.getRepairAppointmentAt() : "Manager đánh dấu mang thiết bị đi kiểm tra thêm";
                addTimeline(req, old, MaintenanceStatus.REPAIR_SCHEDULED, note);
                notifyTenant(req,
                        "Yêu cầu bảo trì — lỗi do khách thuê",
                ...
                addTimeline(req, old, MaintenanceStatus.TENANT_FAULT,
                        "Manager xác định lỗi tenant — cần hẹn lịch sửa");
                notifyTenant(req,
                        "Yêu cầu bảo trì — lỗi do khách thuê",
    ...
    @Override
    @Transactional
    public MaintenanceRequestResponse chargeBeforeRepair(Long id, MaintenanceChargeRequest request) {
        MaintenanceRequest req = findActive(id);
        requireManagerAccess(req);

        if (req.isCompanyAbsorbedFault()) {
            throw new BusinessException(
                    "Phiếu này công ty trả hộ (khách từ chối trả) — không lập hoá đơn thu khách");
        }

        if (req.getChargeInvoiceId() != null) {
            throw new BusinessException("Đã tạo hoá đơn thu phí trước đó");
        }
        ...
        if (request.getDamageCause() != null) {
            req.setDamageCause(request.getDamageCause());
        }
        ...
        repository.save(req);
        addTimeline(req, req.getStatus(), req.getStatus(), "Manager nhập chi phí dự kiến, tạo hoá đơn tạm tính (chờ thanh toán)");
    ...
    @Override
    @Transactional
    public MaintenanceRequestResponse handover(Long id, MaintenanceHandoverRequest request) {
        MaintenanceRequest req = findActive(id);
        requireManagerAccess(req);
    ...
    public int processOverdueSelfRepairs() {
        ...
        // Luồng thanh toán trước khi bàn giao: Quá hạn 3 ngày chưa thanh toán
        LocalDateTime deadline = LocalDateTime.now(VN_ZONE).minusDays(3);
        List<MaintenanceRequest> handoverOverdue = repository.findByStatusAndDeletedFalse(MaintenanceStatus.REPAIR_SCHEDULED).stream()
                .filter(r -> r.getRepairAppointmentAt() != null && r.getRepairAppointmentAt().isBefore(deadline))
                .filter(r -> r.getChargeInvoiceId() != null)
                .toList();

        for (MaintenanceRequest req : handoverOverdue) {
            TenantInvoice invoice = tenantInvoiceRepository.findById(req.getChargeInvoiceId()).orElse(null);
            if (invoice != null && invoice.getStatus() != com.sep490.slms2026.enums.TenantInvoiceStatus.PAID) {
                MaintenanceStatus old = req.getStatus();
                req.setStatus(MaintenanceStatus.OUTSTANDING_DAMAGE);
                repository.save(req);
                createOutstandingDamageRecord(req);
                addTimeline(req, old, MaintenanceStatus.OUTSTANDING_DAMAGE,
                        "Quá hạn bàn giao chưa thanh toán — ghi nhận thiệt hại chờ checkout");
                notifyTenant(req,
                        "Quá hạn thanh toán phí sửa chữa",
                        "Yêu cầu #" + req.getId() + " — chi phí "
                                + req.getInvoiceAmount() + "đ sẽ được xử lý khi checkout.",
                        "MAINTENANCE_SELF_REPAIR_OVERDUE"); // Reuse email type
                count++;
            }
        }
    ...
    @Override
    @Transactional
    public MaintenanceRequestResponse confirmArrival(Long id, MaintenanceConfirmArrivalRequest request) {
        MaintenanceRequest req = findActive(id);
        requireManagerAccess(req);
        requireStatus(req, MaintenanceStatus.OPEN);
        if (req.getVisitArrivalConfirmedAt() != null) {
    ...
    @Override
    @Transactional
    public MaintenanceRequestResponse sendForInspection(Long id, MaintenanceSendForInspectionRequest request) {
        MaintenanceRequest req = findActive(id);
        requireManagerAccess(req);
        requireStatus(req, MaintenanceStatus.OPEN);
        requireVisitArrivalConfirmed(req);
        ...
        if (req.getDamageCause() != null) {
            throw new BusinessException("Phiếu đã có nguyên nhân — không thể gửi đi kiểm tra lại theo luồng này");
        }

        if (request != null && request.getCategory() != null && !request.getCategory().isBlank()) {
            req.setCategory(parseCategoryRequired(request.getCategory()));
        }

        MaintenanceStatus old = req.getStatus();
        req.setStatus(MaintenanceStatus.REPAIR_SCHEDULED);
        req.setAcknowledgedAt(LocalDateTime.now(VN_ZONE));
        // Chưa xác định nguyên nhân / flow — để trống cho tới /diagnose
        req.setFlowType(null);
        req.setDamageCause(null);
        req.setFaultResolutionPath(null);
        req.setCompanyAbsorbedFault(false);
        req.setCompanyAbsorbedNote(null);

        if (request != null && request.getExpectedReturnAt() != null) {
            req.setExpectedReturnAt(request.getExpectedReturnAt());
        }
        ...
        String note = (request != null && !isBlank(request.getNote()))
                ...
                : "Mang thiết bị đi kiểm tra thêm — chưa xác định nguyên nhân";
        if (req.getExpectedReturnAt() != null) {
            note += " (dự kiến trả: " + req.getExpectedReturnAt() + ")";
        }
        addTimeline(req, old, MaintenanceStatus.REPAIR_SCHEDULED, note);
        notifyTenant(req,
                "Thiết bị đang được mang đi kiểm tra",
                "Yêu cầu #" + req.getId() + " \"" + req.getTitle()
                        + "\" — quản lý mang thiết bị đi kiểm tra thêm."
                        + (req.getExpectedReturnAt() != null
                        ? " Dự kiến trả khoảng: " + req.getExpectedReturnAt() : ""),
                "MAINTENANCE_SCHEDULE_CHANGED");
        realtimeEventService.publishMaintenanceEvent(req, RealtimeEventService.EVT_MAINTENANCE_SCHEDULE_CHANGED);
        return convertToResponse(req);
    }
    ...
    @Override
    @Transactional
    public MaintenanceRequestResponse diagnose(Long id, MaintenanceDiagnoseRequest request) {
        MaintenanceRequest req = findActive(id);
        requireManagerAccess(req);
        requireVisitArrivalConfirmed(req);

        boolean fromOpen = req.getStatus() == MaintenanceStatus.OPEN;
        boolean fromInspection = req.getStatus() == MaintenanceStatus.REPAIR_SCHEDULED
                && req.getDamageCause() == null;
        if (!fromOpen && !fromInspection) {
            throw new BusinessException(
                    "Chỉ chẩn đoán khi phiếu OPEN (sửa ngay) hoặc REPAIR_SCHEDULED chưa có nguyên nhân. Hiện tại: "
                            + req.getStatus()
                            + (req.getDamageCause() != null ? " (đã có nguyên nhân)" : ""));
        }

        if (request == null || request.getDamageCause() == null) {
            throw new BusinessException("damageCause là bắt buộc: WEAR hoặc TENANT_MISUSE");
        }
        DamageCause cause = request.getDamageCause();
        ...
        String categoryFromBody = request.getCategory();
        if (categoryFromBody != null && !categoryFromBody.isBlank()) {
            req.setCategory(parseCategoryRequired(categoryFromBody));
        } else if (req.getCategory() == null || req.getCategory().isBlank()) {
            throw new BusinessException("Danh mục sự cố (category) là bắt buộc khi chẩn đoán");
        }
        String priority = parsePriorityOptional(request.getPriority());
        if (priority != null) {
            req.setPriority(priority);
        }
        ...
        MaintenanceStatus old = req.getStatus();
        LocalDateTime repairAt = request.getRepairAppointmentAt();
        String amountNote = formatDiagnoseAmountNote(req, needsReplacement);

        if (cause == DamageCause.WEAR) {
            req.setFlowType(MaintenanceFlowType.NORMAL_WEAR);
            req.setDamageCause(DamageCause.WEAR);
            req.setFaultReason(null);
            req.setFaultResolutionPath(null);
            req.setCompanyAbsorbedFault(false);
            req.setCompanyAbsorbedNote(null);
            ...
            if (fromInspection || repairAt != null) {
                ...
                notifyTenant(req,
                        "Đã có kết quả kiểm tra — công ty chịu chi phí",
                        "Yêu cầu #" + req.getId() + " xác định hao mòn tự nhiên. Lịch giao/sửa: " + repairAt,
                        "MAINTENANCE_APPROVED");
                realtimeEventService.publishMaintenanceEvent(req, RealtimeEventService.EVT_MAINTENANCE_APPROVED);
                realtimeEventService.publishMaintenanceEvent(req, RealtimeEventService.EVT_MAINTENANCE_SCHEDULE_CHANGED);
            } else {
                ...
                repository.save(req);
                addTimeline(req, old, MaintenanceStatus.IN_REPAIR,
                        "Chẩn đoán: hao mòn tự nhiên — sửa ngay, " + amountNote);
                notifyTenant(req,
                        "Yêu cầu bảo trì đã được duyệt",
                        "Yêu cầu #" + req.getId() + " \"" + req.getTitle() + "\" — hao mòn tự nhiên, đang sửa chữa.",
                        "MAINTENANCE_APPROVED");
                realtimeEventService.publishMaintenanceEvent(req, RealtimeEventService.EVT_MAINTENANCE_APPROVED);
            }
            return convertToResponse(req);
        ...
        req.setDamageCause(DamageCause.TENANT_MISUSE);
        req.setFaultReason(request.getFaultReason().trim());
        req.setFaultResolutionPath(FaultResolutionPath.MANAGER_REPAIR);
        ...
        if (companyAbsorbs) {
            ...
            if (fromInspection || repairAt != null) {
                ...
                repository.save(req);
                addTimeline(req, old, MaintenanceStatus.REPAIR_SCHEDULED,
                        "Chẩn đoán: lỗi khách — khách từ chối trả, công ty trả hộ. " + amountNote
                                + (repairAt != null ? ", hẹn giao " + repairAt : ""));
                notifyTenant(req,
                        "Kết quả kiểm tra — công ty tạm ứng chi phí",
                        "Yêu cầu #" + req.getId() + " xác định lỗi do sử dụng. Công ty tạm ứng sửa chữa.",
                        "MAINTENANCE_TENANT_FAULT");
            } else {
                req.setStatus(MaintenanceStatus.IN_REPAIR);
                markRoomMaintenance(req);
                repository.save(req);
                addTimeline(req, old, MaintenanceStatus.IN_REPAIR,
                        "Chẩn đoán: lỗi khách — khách từ chối trả, công ty trả hộ. Sửa ngay, " + amountNote);
                notifyTenant(req,
                        "Công ty tạm ứng chi phí sửa chữa",
                        "Yêu cầu #" + req.getId() + " — lỗi do sử dụng, công ty tạm ứng sửa chữa.",
                        "MAINTENANCE_TENANT_FAULT");
            }
            realtimeEventService.publishMaintenanceEvent(req, RealtimeEventService.EVT_MAINTENANCE_REJECT_FAULT);
            if (req.getStatus() == MaintenanceStatus.REPAIR_SCHEDULED) {
    ...
    public MaintenanceRequestResponse rescheduleRepair(Long id, MaintenanceRescheduleRepairRequest request) {
        ...
        String note = previous != null 
                ...
                : "Đặt lịch sửa/bàn giao sau kiểm tra: " + request.getRepairAppointmentAt();
        addTimeline(req, MaintenanceStatus.REPAIR_SCHEDULED, MaintenanceStatus.REPAIR_SCHEDULED, note);
        notifyTenant(req,
    ...
    public MaintenanceRequestResponse startRepair(Long id) {
        ...
        if (req.isCompanyAbsorbedFault() || req.getFlowType() != MaintenanceFlowType.TENANT_FAULT) {
            next = MaintenanceStatus.IN_REPAIR;
        } else {
            next = MaintenanceStatus.TENANT_FAULT;
        }
        req.setStatus(next);
    ...
    private void requireVisitArrivalConfirmed(MaintenanceRequest req) {
        ...
        if (req.getVisitArrivalConfirmedAt() == null) {
            throw new BusinessException(
                    "Cần quét QR thiết bị (confirm-arrival) trước khi bắt đầu xử lý phiếu. Xem phiếu không cần quét.");
        }
    }
    ...
    private void recordEquipmentMaintenanceHistory(MaintenanceRequest req) {
        if (req.getEquipmentId() == null) {
            return;
        }
        if (equipmentHistoryRepository.existsByMaintenanceRequestId(req.getId())) {
            return;
        }
        ...
        if (!req.isEquipmentReplacementFlagged()) {
            ...
            equipment.setLastMaintenanceDate(LocalDateTime.now(VN_ZONE));
        ...
        photos.addAll(loadPhotoUrlsByType(req.getId(), MaintenancePhotoType.BEFORE));
        photos.addAll(loadPhotoUrlsByType(req.getId(), MaintenancePhotoType.AFTER));
        photos.addAll(loadPhotoUrlsByType(req.getId(), MaintenancePhotoType.INVOICE));
        List<String> uniquePhotos = photos.stream().filter(u -> u != null && !u.isBlank()).distinct().toList();
        BigDecimal cost = req.getInvoiceAmount() != null ? req.getInvoiceAmount() : req.getEstimatedDamageAmount();
        EquipmentMaintenanceHistory history = EquipmentMaintenanceHistory.builder()
                .equipment(equipment)
                .maintenanceRequest(req)
                .maintenanceDate(req.getDoneAt() != null ? req.getDoneAt() : LocalDateTime.now(VN_ZONE))
                .repairCost(cost != null ? cost.longValue() : null)
                .note(req.getRepairDescription() != null ? req.getRepairDescription() : req.getResolutionNote())
                .photoUrls(uniquePhotos.isEmpty() ? null : String.join(",", uniquePhotos))
                .build();
    ...
    private ManagerAvailabilitySlotResponse toAvailabilitySlot(
        ...
        if (roomNumber == null && r.getEquipmentId() != null) {
            roomNumber = equipmentRepository.findById(r.getEquipmentId())
                    .map(eq -> eq.getRoom() != null ? eq.getRoom().getRoomNumber() : null)
                    .orElse(null);
        }
    ...
    private BigDecimal resolveMaintenanceChargeAmount(MaintenanceRequest req, boolean needsReplacement) {
        ...
        if (req.getInvoiceAmount() != null && req.getInvoiceAmount().compareTo(BigDecimal.ZERO) > 0) {
            return req.getInvoiceAmount();
        }
        if (req.getEstimatedDamageAmount() != null && req.getEstimatedDamageAmount().compareTo(BigDecimal.ZERO) > 0) {
            return req.getEstimatedDamageAmount();
        }
    ...
    private boolean hasMaintenanceCharge(Long requestId) {
        return !tenantPendingChargeRepository
                .findByMaintenanceRequestIdOrderByCreatedAtDesc(requestId)
                .isEmpty();
    }
    ...
    private MaintenanceRequestResponse convertToResponse(MaintenanceRequest req) {
        ...
        MaintenanceRequestResponse res = MaintenanceRequestResponse.builder()
                ...
                .companyAbsorbedFault(req.isCompanyAbsorbedFault())
                .companyAbsorbedNote(req.getCompanyAbsorbedNote())
                .expectedReturnAt(req.getExpectedReturnAt())
                .equipmentReplacementFlagged(req.isEquipmentReplacementFlagged())
                .beforeImages(before)
                .afterImages(after)
