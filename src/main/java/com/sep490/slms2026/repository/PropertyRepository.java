...
public interface PropertyRepository extends JpaRepository<Property, Long> {
    ...
    @Modifying
    @Query("UPDATE Property p SET p.operationManagerId = :managerId, p.operationManagerId = :managerId, p.status = CASE WHEN p.status = 'PENDING_OPERATION_MANAGER' THEN 'ACTIVE' ELSE p.status END WHERE p.zone.id = :zoneId AND p.status IN :validStatuses")
    ...
    @Modifying
    @Query("UPDATE Property p SET p.operationManagerId = null, p.operationManagerId = null, p.status = CASE WHEN p.status = 'ACTIVE' THEN 'PENDING_OPERATION_MANAGER' ELSE p.status END WHERE p.zone.id = :zoneId AND p.status IN :validStatuses")
